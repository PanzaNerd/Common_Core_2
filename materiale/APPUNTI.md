...
- **★ Perché `end=""` + `if not content.endswith("\n")` (★ PRIMA VOLTA: il parametro `end` di print):** la riga di chiusura `--File ... closed.` deve SEMPRE stare su una riga sua, con ESATTAMENTE un a-capo tra contenuto e chiusura — né zero né due. Ma non si sa in anticipo se il file finisce con l'a-capo o no. La coppia: `end=""` TOGLIE l'a-capo automatico di print (quello del contenuto, se c'è, resta) · `if not content.endswith("\n")` = domanda "il contenuto NON finisce con a-capo?" → `print()` vuoto RIMETTE quello che manca. Uno toglie sempre, l'altro rimette solo se serve → sempre esattamente uno. `end=""` NON innesca l'if: sono due pezzi indipendenti (l'if guarda il FILE, non il print). E NON si può omettere `end=""`: senza, col file che finisce con a-capo esce UNA RIGA VUOTA in più (a-capo del contenuto + a-capo di print = 2) e l'output non è più quello del subject. Dimostrato dal vero con 4 varianti (file con/senza a-capo finale × codice con/senza la coppia). ex1 e ex2 riusano lo stesso pezzo identico
...
**Teoria:** l'oggetto file ha DUE metodi speculari: `.read()` (legge tutto) e `.write(testo)` (scrive il testo). La modalità si sceglie nel secondo parametro di open: "r" legge, "w" scrive (crea/sovrascrive). In C: `fopen(name, "r")` / `fopen(name, "w")` + `fwrite`. Il print con `end=""` + `if not endswith` (la coppia che garantisce un solo a-capo prima della chiusura) è spiegato in ex0, dove compare per la prima volta.
